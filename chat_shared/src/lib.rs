...
impl Message {
    pub fn new(msg_type: MessageTypes, content: String, id: u8) -> Self {
        Message {
            id,
            msg_type,
            length: content.len(),
    ...
    pub fn get_content(&self) -> Result<String, std::str::Utf8Error> {
        std::str::from_utf8(self.content.as_bytes()).map(|s| s.to_string())
    }
}
...
impl TryFrom<Message> for Vec<u8> {
    ...
    fn try_from(message: Message) -> Result<Self, Self::Error> {
        ...
        buffer.extend_from_slice(message.content.as_bytes());
        Ok(buffer)
    }
}
